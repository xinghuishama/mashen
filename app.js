// ======================== app.js — 主线程核心逻辑 v3.5 (重构) ========================
// 设计目标：APK WebView 100% 兼容 · Worker 全量卸载 · 离线缓存 · 无 inline JS · 并发安全 · 资源清理
(function () {
  "use strict";
  ...
  // 合法筛选条件白名单（用于校验 localStorage 恢复）
  const VALID_FILTERS = {
    shengxiao: SHENGXIAO ? Object.keys(SHENGXIAO).map(s => '生肖' + s) : [],
    haomatou: [ '0头单','0头双','1头单','1头双','2头单','2头双','3头单','3头双','4头单','4头双' ],
    weishu: [ '0尾','1尾','2尾','3尾','4尾','5尾','6尾','7尾','8尾','9尾' ],
    shuduan: [ '1段','2段','3段','4段','5段','6段','7段' ],
    bose: [ '红波单','红波双','蓝波单','蓝波双','绿波单','绿波双' ],
    wuxing: [ '金','木','水','火','土' ],
    bandanshuang: [ '合数单','合数双','大单','大双','小单','小双' ],
    heshu: Array.from({ length: 13 }, (_, i) => (i + 1) + '合')
  };
  ...
  function clearAllFilters() {
    state.killNums = [];
    for (const k of Object.keys(state.selectedFilters)) {
  ...
  function loadState() {
    try {
      ...
      // selectedFilters：深拷贝数组，且每个值必须属于对应分类白名单
      if (parsed.selectedFilters && typeof parsed.selectedFilters === 'object') {
        for (const k of Object.keys(state.selectedFilters)) {
          const val = parsed.selectedFilters[k];
          if (Array.isArray(val)) {
            const whitelist = VALID_FILTERS[k] || [];
            state.selectedFilters[k] = val.filter(v => whitelist.includes(v));
          }
        }
      }
    } catch (e) {
  ...
  function initWorker() {
    ...
    try {
      ...
      analysisWorker.onerror = e => console.error('Worker error:', e);
      // 向 Worker 发送一次静态数据（numProps），之后只传输入，减少传输量
      analysisWorker.postMessage({ type: 'init', numProps });
    } catch (e) {
      ...
      try {
        ...
        if (analysisWorker) {
          analysisWorker.postMessage({
            ...
            // 不再传输 numProps，Worker 已通过 init 消息获取
          });
        }
      } catch (err) {
  ...
  // ======================== 开奖数据获取（并发锁 + 离线缓存）========================
  let isFetchingLottery = false; // 互斥锁
  ...
  async function fetchLottery() {
    if (isFetchingLottery) return; // 锁保护
    const btn = DOM.refreshLotteryBtn;
    if (!btn) return;
    isFetchingLottery = true;
    const origHtml = btn.innerHTML;
    btn.innerHTML = '<svg class="animate-spin w-5 h-5" ...>...</svg>加载中...';
    btn.disabled = true;
    try {
  ...
  // 翻页方法（作为模块内部方法）
  function prevHistoryPage() { if (currentHistoryPage > 1) { currentHistoryPage--; renderHistoryPage(); } }
  function nextHistoryPage() { ensureHistorySorted(); const totalPages = Math.ceil(currentHistorySorted.length / HISTORY_PAGE_SIZE); if (currentHistoryPage < totalPages) { currentHistoryPage++; renderHistoryPage(); } }
  ...
  history: function () {
    ...
    open: function (type) {
      ...
      // 关闭旧抽屉并清理直播资源
      if (this.current === 'live') destroyLivePlayer();
      this.current = type;
      const titles = { shama:'杀码', shengxiao:'生肖', /*...*/ live:'开奖直播', history:'历史开奖' };
    ...
    bindEvents: function (type) {
      ...
      if (type === 'history') {
        ...
        // 翻页按钮事件代理（避免重复绑定）
        const pagi = document.getElementById('historyPagination');
        if (pagi && !pagi._listener) {
          pagi.addEventListener('click', e => {
            const btn = e.target.closest('button');
            if (!btn) return;
            if (btn.id === 'history-prev') prevHistoryPage();
            else if (btn.id === 'history-next') nextHistoryPage();
          });
          pagi._listener = true;
        }
      }
  ...
  function copyResult() {
    ...
    if (text.trim()) {
      navigator.clipboard && navigator.clipboard.writeText(text.trim())
        .then(() => showToast('已复制结果'))
        .catch(() => showToast('复制失败'));
    }
  }

  function copyNumber(n) {
    navigator.clipboard && navigator.clipboard.writeText(String(n).padStart(2, '0'))
      .then(() => showToast('已复制 ' + n))
      .catch(() => showToast('复制失败'));
  }
  window.copyResult = copyResult; // 保留全局便于外部调用，但也可移除
          ...
          .then(data => {
            if (data && data[0] && data[0].videoUrl) {
              ...
              if (idx + 1 < LIVE_SOURCES.length) {
                ...
                setTimeout(() => connectLiveSource(idx + 1), 1000);
              } else {
                showLiveError();
              }
            }
  ...
  function init() {
    ...
    console.log('%c✅ 神码再现 v3.5 已加载（APK WebView 终极优化版）', 'color:#00ffea;font-weight:bold');
  }
